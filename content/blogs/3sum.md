...
3. You move the pointers based on whether the sum is less than, greater than, or equal to zero.
4. you also skip duplicates to ensure unique triplets.
...
```java
for(int i=0;i<n-2;i++){ //index i will be the anchor
...
while(left < right){
    ...
    if(sum == 0){
        ...
        while(left < right && nums[right]==nums[right-1]){ //Skipping duplicate
            right--;
        }
        left++;
        right--;
